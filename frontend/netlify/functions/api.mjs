...
function normalizePath(pathname = '') {
  if (pathname.startsWith('/.netlify/functions/api')) {
    const nextPath = pathname.replace('/.netlify/functions/api', '');
    return nextPath || '/';
  }

  if (pathname.startsWith('/api')) {
    const nextPath = pathname.replace('/api', '');
    return nextPath || '/';
  }
...
function normalizeContentRecords(value) {
  if (!Array.isArray(value)) {
    return cloneJson(defaultContent);
  }

  return value
    .filter((item) => item && typeof item === 'object' && typeof item.content_id === 'string')
    .map((item) => ({
      ...item,
...
function normalizePortfolioRecords(value) {
  if (!Array.isArray(value)) {
    return [];
  }
  ...
  return value
    ...
    .map((item, index) => ({
      project_id: item.project_id || crypto.randomUUID(),
      title: item.title || '',
      description: item.description || '',
      full_description: item.full_description || '',
      project_type: item.project_type === 'app' ? 'app' : 'website',
      video_url: item.video_url || '',
      seo_keywords: Array.isArray(item.seo_keywords) ? item.seo_keywords.filter(Boolean) : [],
...
function normalizeContactSubmissions(value) {
...
async function readPersistentValue(key, fallbackValue) {
  try {
    ...
    if (process.env.NETLIFY || process.env.CONTEXT || process.env.SITE_ID) {
      throw new Error(
        `Persistent storage is unavailable on Netlify for "${key}". Configure MongoDB if this deploy cannot access Netlify Blobs.`
      );
    }
...
async function writePersistentValue(key, value) {
  ...
  try {
    ...
    if (process.env.NETLIFY || process.env.CONTEXT || process.env.SITE_ID) {
      throw new Error(
        `Persistent storage write failed on Netlify for "${key}". Configure MongoDB if this deploy cannot access Netlify Blobs.`
      );
    }
...
async function getContentRecords() {
  ...
  if (db) {
    ...
    const records = await db.collection('editable_content').find({}, { projection: { _id: 0 } }).toArray();
    return normalizeContentRecords(records);
  }

  const records = await readPersistentValue(CONTENT_KEY, defaultContent);
  return normalizeContentRecords(records);
}
...
async function getPortfolioRecords() {
  ...
  const records = await readPersistentValue(PORTFOLIO_KEY, []);
  return normalizePortfolioRecords(records);
}
...
async function handleUpdateContent(request, pathParts) {
...
async function handleUpdatePortfolio(request, pathParts) {
  const authError = requireAdmin(request);
  if (authError) {
    return authError;
  }
...
async function handleDeletePortfolio(request, pathParts) {
  const authError = requireAdmin(request);
  if (authError) {
    return authError;
  }
...
async function handleInitializeContent(request) {
  const authError = requireAdmin(request);
  if (authError) {
    return authError;
  }
...
async function handleRoot() {
  return jsonResponse(200, {
    ...
    persistenceMode: process.env.MONGO_URL ? 'mongodb' : 'netlify-blobs',
    persistenceConsistency: 'strong',
...
export default async (request) => {
  try {
    ...
    if (pathParts[0] === 'content') {
      ...
      if (method === 'PUT') {
        return handleUpdateContent(request, pathParts);
      }
    }
    ...
    if (normalizedPath === '/portfolio' && method === 'POST') {
      return handleCreatePortfolio(request);
    }

    if (pathParts[0] === 'portfolio' && pathParts[1] && method === 'PUT') {
      return handleUpdatePortfolio(request, pathParts);
    }

    if (pathParts[0] === 'portfolio' && pathParts[1] && method === 'DELETE') {
      return handleDeletePortfolio(request, pathParts);
    }
    ...
    if (normalizedPath === '/init-content' && method === 'POST') {
      return handleInitializeContent(request);
    }
